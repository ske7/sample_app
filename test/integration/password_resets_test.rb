...
class PasswordUpdateTest < PasswordResetForm
  test 'update with invalid password and confirmation' do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email, user: { password: 'password', password_confirmation: '123456' } }
  ...
  test 'update with valid password and confirmation' do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email, user: { password: 'password', password_confirmation: 'password' } }
...
class ExpiredTokenTest < ExpiredToken
  ...
  test "should include the word 'expired' on the password-reset page" do
    ...
    assert_match (/expired/i), response.body # rubocop:disable Style/RedundantParentheses
  end
end
